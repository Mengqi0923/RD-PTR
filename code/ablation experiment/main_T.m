...
for i=1:size(newChrom1, 1)
    for j=1:num_taxi
        if Chrom2(i,(j-1)*max_capacity*2+1) ~= 0   % && Chrom2(i,(j-1)*max_capacity*2+3) == 0
            orderid = Chrom2(i,(j-1)*max_capacity*2+1);
...
%------------------------------------------------------------    
    ...
    % 修正函数
    [newChrom1, newChrom2, newChrom3] = correction_T(newChrom1, newChrom2, newChrom3, distances_km, sets, num_taxi, max_capacity, max_detour, original_distances, min_time, max_time, pick_up_time, v_taxi);
    newChrom3 = ones(pop_size, num_order*2);
    % 计算目标函数值
