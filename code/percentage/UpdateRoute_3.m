function [newChrom2, IsChange] = UpdateRoute_3(Chrom1, Chrom2, newChrom1, newChrom3, pick_up_time, num_order, num_taxi, max_capacity, p2, distances, v_taxi, sets, IsChange)
%UpdateRoute 更新种群第二部分（路径）
%   仅限于最多允许两辆车拼单
newChrom2 = Chrom2;
newtime = pick_up_time + newChrom1;   % 更新后的上车时间
isChange = newChrom1 - Chrom1;
new_max_capacity = max_capacity * 2;
for i=1:size(newChrom2,1)
    if ~all(isChange(i,:)==0)   % 该个体有订单上车时间改变，则需重新调整订单上下车点的顺序
        against_order = [];
        for j=i:num_taxi
            route = newChrom2(i, (j-1)*new_max_capacity+1:j*new_max_capacity);
            route = route(route>0);
            if length(route)>2
                k = 2;
                while k<=length(route)
                    front_r = route(1:k-1);   % 计算时间，判断该订单是否违反时间约束
                    time_front = CaculateTime(front_r, num_order, newtime(i,:),distances, v_taxi, sets, newChrom3(i,:));
                    actual_time = time_front + distances(sets{route(k-1)}(newChrom3(i,route(k-1))),sets{route(k)}(newChrom3(i,route(k)))) / v_taxi;
                    if route(k)<=num_order
                        if actual_time>newtime(i,route(k))   % 违反时间约束
                            tag = 0;   % 标记是否调整了违反约束的订单，0为否，1为是
                            for kk=(k-1):-1:1
                                extime = CaculateTime(route(kk), num_order, newtime(i,:),distances, v_taxi, sets, newChrom3(i,:)) + distances(sets{route(kk)}(newChrom3(i,route(kk))),sets{route(k)}(newChrom3(i,route(k)))) / v_taxi;
            ...
            newChrom2(i, (j-1)*new_max_capacity+1:(j-1)*new_max_capacity+length(route)) = route;
            newChrom2(i, (j-1)*new_max_capacity+length(route)+1:j*new_max_capacity) = 0;

        end
        if ~isempty(against_order)   % 存在时间违约订单
            k = 1;
            for j=1:num_taxi
                if all(newChrom2(i, (j-1)*new_max_capacity+1:j*new_max_capacity) == 0)
                    newChrom2(i, (j-1)*new_max_capacity+1:(j-1)*new_max_capacity+2) = [against_order(k), against_order(k) + num_order];
    ...
    if rand < p2 && IsChange(i)
        ...
        for j=1:num_taxi
            ...
            if length(sub_route) > 2   % 交换同一辆车中不同订单的下车点和上车点的先后顺序
                if route(2)>num_order && route(3)<=num_order % 若前一个是下车点，后一个是上车点——1 3 2 4
                    if route(2) ~= route(3) + num_order
                        d32 = distances(sets{route(1)}(newChrom3(i,route(1))), sets{route(3)}(newChrom3(i,route(3)))) + distances(sets{route(3)}(newChrom3(i,route(3))), sets{route(2)}(newChrom3(i,route(2))));
                ...
                elseif route(2) <= num_order && route(3) > num_order
                    if route(2) + num_order == route(3)
                        d34 = distances(sets{route(2)}(newChrom3(i,route(2))), sets{route(3)}(newChrom3(i,route(3)))) + distances(sets{route(3)}(newChrom3(i,route(3))), sets{route(4)}(newChrom3(i,route(4))));
                        d43 = distances(sets{route(2)}(newChrom3(i,route(2))), sets{route(4)}(newChrom3(i,route(4)))) + distances(sets{route(4)}(newChrom3(i,route(4))), sets{route(3)}(newChrom3(i,route(3))));
                        if d43 < d34
            ...
            newChrom2(i, (j-1)*new_max_capacity+1:j*new_max_capacity) = route;
            temp = newChrom2(i, (j-1)*new_max_capacity+1:j*new_max_capacity);
        ...
        for j=1:num_taxi
            ...
            newChrom2(i, (j-1)*new_max_capacity+1:j*new_max_capacity) = [routes{j}, x];
        end
        IsChange(i) = 1;
    end
