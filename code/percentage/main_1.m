...
% v_taxi = 1000/3;                   % 出租车速度 20km/h
% v_taxi = 2000/3;                   % 出租车速度 40km/h
...
%% 不同比例接受推荐点的乘客
uo_r=[0, 0.25, 0.5, 0.75, 1];
time_r=[0, 0.25, 0.5, 0.75, 1];
uo_p = uo_r(5);
time_p = time_r(1);
...
uo_perenfence1 = randsrc(1,num_order,[0,1;(1-uo_p), uo_p]);
uo_perenfence = [uo_perenfence1, uo_perenfence1];
t_perenfence = randsrc(1,num_order, [0, 1; (1-time_p), time_p]);
...
% 计算原价
[original_fare, original_distances] = originFare(sets,distances,num_order,cF, min_orderfare, min_distance);
% % --------------------------------------------------------
...
%         original_fare(i) = (original_distances(i) - min_distance) * cF + min_orderfare;                               % 计算原价
%     else
%         original_fare(i) = min_orderfare;
%     end
% end
% % --------------------------------------------------------

Chrom1 = init1(pop_size, num_order, min_time, max_time,t_perenfence);  % 上车时间种群
Chrom3 = init3(sets, pop_size, num_order, uo_perenfence);  % 上下车点种群
...
while iter <= maxG
    ...
    [newChrom1, newv, IsChange] = UpdateTime_1(Chrom1, Chrom2, num_taxi, max_capacity, min_time, max_time, paretoSet, v, w, c1, min_v, max_v, p1, IsChange, t_perenfence);   % 更新种群1   
    [newChrom3, IsChange] = UpdatePoint(Chrom3, sets, p3, IsChange, uo_perenfence);                                                         % 更新种群3
...
% 
% [row,col] = find(distances==inf);
% for i=1:length(row)
%    distances(row(i),col(i)) = distances(col(i),row(i));
% end
% % distances(row,col) = distances(col,row);
% 
% 
%  fid=fopen('distances_1min_39.txt','w');
%     [m,n]=size(distances);
%     for i=1:m
%         for j=1:n
%             if j==n
