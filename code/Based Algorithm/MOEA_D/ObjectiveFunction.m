function [total_fare,unit_profit] = ObjectiveFunction(Chrom1, Chrom2, Chrom3, alpha, bata, lambd, sets, num_order, num_taxi, max_capacity, original_fare, original_distances, distances_km ,cE, max_walk, max_detour)
%payment 支付金额
%   输入：
% Chrom1, Chrom2, Chrom3三个分种群
% cF  单位里程价格
% alpha  绕道折扣系数
% bata  步行折扣系数
% lambd  时间折扣系数
...
for i=1:len
    e = zeros(num_order,1);
    % 使用reshape分割向量为多个子向量
%     subVectors = reshape(Chrom2(i,:),[max_capacity*2, num_taxi])';
    for j=1:num_taxi
        s = Chrom2(i,(j-1)*max_capacity*2+1:j*max_capacity*2);
        s1 = s(s ~= 0);   % 删除0元素
            ...
            for k=1:length(s1)-1
                ...
                if s1(k)<=num_order
                    ...
                    for kk=k+1:length(s1)
                        pu1 = sets{s1(k1)};
                        do = sets{s1(kk)};
                        newdistance = newdistance + distances_km(pu1(Chrom3(i,s1(k1))), do(Chrom3(i,s1(kk))));
                        k1 = k1 + 1;
                    ...
                    l_walk(i,s1(k)) = distances_km(pu(1),pu(Chrom3(i,s1(k)))) + distances_km(do(1),do(Chrom3(i,s1(kk))));
                    e(s1(k)) = Fare(original_fare(s1(k)), alpha, delta(i,s1(k)), bata, l_walk(i,s1(k)), lambd, Chrom1(i,s1(k)), max_detour, max_walk_km);
                end
                % 计算出租车总距离
                total_distance(i) = total_distance(i) + distances_km(sets{s1(k)}(Chrom3(i,s1(k))),sets{s1(k+1)}(Chrom3(i,s1(k+1))));
            end
