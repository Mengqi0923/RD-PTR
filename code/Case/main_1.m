...
tic
%% 模型数据
points = importdata("points_10min_20.txt");   % 所有集合中的坐标点，包括了原上下车点
% sets_index = importdata("merged_set_index.txt");
distances = importdata("distances_10min_20.txt");   % 距离矩阵
distances_km = distances/1000;

% 读取txt文件
filename = 'merged_set_index_10min_20.txt';
...
% 原上车时间
pick_up_time = importdata("pickup_time_10min_20.txt")'; % 上车时间
...
% hold on;
scatter(initprofit, initfare, 'b', 'MarkerFaceColor','b', 'MarkerFaceAlpha',.3,'MarkerEdgeAlpha',.8)
hold on;
plot(C(:,1), C(:,2), 'r*', 'MarkerSize', 10);
hold on;
scatter(cF-new_Pareto(:,2), new_Pareto(:,1), 'y')
hold on;
plot(cF-new_Pareto(med_I,2), new_Pareto(med_I,1), 'rp', 'MarkerFaceColor','r', 'MarkerSize', 10);
